...
const panelInner = user ? buildLoggedInPanel(user) : buildAuthPanel();
...
function buildLoggedInPanel(u) {
    const initial = u.name ? u.name.charAt(0).toUpperCase() : '?';
    return `
    <div class="auth-panel-body auth-user-info">
      <div class="auth-user-avatar">${initial}</div>
      <span class="auth-user-status">Logged In</span>
      <p class="auth-user-name">${escapeHtml(u.name)}</p>
      <p class="auth-user-email">${escapeHtml(u.email)}</p>
      <div class="auth-user-divider"></div>
      <a href="/profile" class="auth-user-link">My Profile</a>
      <a href="/wishlist" class="auth-user-link">Wishlist</a>
      <a href="/cart" class="auth-user-link">My Bag</a>
      <div class="auth-user-divider"></div>
      ...
      <form class="auth-form" data-form="register" action="/register" method="POST" novalidate>
        ...
        <div class="auth-row">
          <div class="auth-field">
            ...
            <input type="text" id="regFirst" name="first_name" placeholder="John" required autocomplete="given-name" />
            <small class="auth-field-error" data-error-for="first_name"></small>
          </div>
          <div class="auth-field">
            <label for="regLast">Last Name</label>
            <input type="text" id="regLast" name="last_name" placeholder="Smith" required autocomplete="family-name" />
            <small class="auth-field-error" data-error-for="last_name"></small>
          </div>
        </div>
        <div class="auth-field">
          <label for="regEmail">Email</label>
          <input type="email" id="regEmail" name="email" placeholder="your@email.com" required autocomplete="email" />
          <small class="auth-field-error" data-error-for="email"></small>
        </div>
        <div class="auth-field">
        ...
        <div class="auth-field">
          ...
          <input type="password" id="regConfirm" name="password_confirmation" placeholder="••••••••" required autocomplete="new-password" />
...
if (accountBtn) accountBtn.addEventListener('click', (e) => { e.preventDefault(); openAuth('login'); });
authClose.addEventListener('click', closeAuth);
authOverlay.addEventListener('click', closeAuth);
document.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeAuth(); });

// ── Below only runs for guest panel ──────────────────────────────────────────
...
if (!user) {
    ...
    function clearErrors() {
        ...
        document.querySelectorAll('.auth-field input').forEach(input => { input.classList.remove('is-invalid'); });
    }
    ...
    document.querySelectorAll('.auth-tab').forEach(tab => {
        tab.addEventListener('click', () => { clearErrors(); switchTab(tab.dataset.tab); });
    });
